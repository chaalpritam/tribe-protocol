...
pub struct RotateAppKey<'info> {
    ...
    #[account(
        mut,
        seeds = [b"app_key", &tid_record.try_borrow_data()?[8..16], old_app_key_record.app_pubkey.as_ref()],
    ...
    #[account(
        ...
        seeds = [b"app_key", &tid_record.try_borrow_data()?[8..16], new_app_pubkey.as_ref()],
        bump,
    )]
